...
fn layouts() -> Vec<VerifiedLayout> {
    layout::parse(include_str!("../tests/fixtures/direct450/layouts.json")).unwrap()
}

fn cases() -> Vec<Value> {
    serde_json::from_str(include_str!("../tests/fixtures/direct450/cases.json")).unwrap()
}

fn captured(case: &Value) -> eth::Block {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/direct450");
    eth::Block::decode(std::fs::read(root.join(case["fixture"].as_str().unwrap())).unwrap().as_slice()).unwrap()
}

#[test]
fn direct450_captured_blocks_match_independent_rpc() {
    let layouts = layouts();
...
#[test]
fn direct450_fixed_fields_allowances_and_records_preserve_the_independent_rpc_balance() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/direct450/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn direct450_raw_word_controls_match_rpc_including_null_address_filtering() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/direct450/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn direct450_unreviewed_mapping_writes_still_stop_processing() {
    let layouts = layouts();
    for case in cases() {
        let contract = hex_bytes(case["contract"].as_str().unwrap()).unwrap();
        let layout = layouts.iter().find(|l| l.contract == contract).unwrap();
...
#[test]
fn direct450_record_width_and_unknown_fixed_fields_remain_guarded() {
    ...
    let token: Value = serde_json::from_str::<Value>(include_str!("../tests/fixtures/direct450/controls.json")).unwrap()["tokens"]
        .as_array()
        .unwrap()
        .iter()
        .find(|t| t["rank"] == 431)
    ...
    let contract = hex_bytes(token["contract"].as_str().unwrap()).unwrap();
    let layout = configured.iter().find(|l| l.contract == contract).unwrap();
    let controls = token["metadata_controls"].as_array().unwrap();
    let control = controls.iter().rev().find(|c| c["kind"] == "four_word_record").unwrap();
    let mut key: BigInt = BigInt::from_unsigned_bytes_be(&hex_bytes(control["storage_key"].as_str().unwrap()).unwrap());
    key += BigInt::from(1);
    let mut encoded = key.to_bytes_be().1;
    let mut padded = vec![0; 32 - encoded.len()];
