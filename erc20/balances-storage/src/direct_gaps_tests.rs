...
#[test]
fn source_led_direct_mappings_match_captured_independent_rpc() {
    let layouts = layout::parse(include_str!("../tests/fixtures/direct-gaps/layouts.json")).unwrap();
    assert_eq!(layouts.len(), 2);
    let cases: Vec<Value> = serde_json::from_str(include_str!("../tests/fixtures/direct-gaps/cases.json")).unwrap();
    ...
    for case in cases {
        ...
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/direct-gaps")
