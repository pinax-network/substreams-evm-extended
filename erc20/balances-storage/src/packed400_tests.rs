...
fn layouts() -> Vec<VerifiedLayout> {
    layout::parse(include_str!("../tests/fixtures/packed400/layouts.json")).unwrap()
}

fn cases() -> Vec<Value> {
    serde_json::from_str(include_str!("../tests/fixtures/packed400/cases.json")).unwrap()
}

fn captured(case: &Value) -> eth::Block {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/packed400");
    eth::Block::decode(std::fs::read(root.join(case["fixture"].as_str().unwrap())).unwrap().as_slice()).unwrap()
}

#[test]
fn packed_captured_blocks_need_reviewed_fields_and_match_independent_rpc() {
    let layouts = layouts();
    ...
    for case in cases() {
        ...
        unreviewed.other_slots.clear();
        let error = project(&block, &[unreviewed]).unwrap_err().to_string();
        assert!(error.contains("unresolved storage"));
        let scalar = if contract == hex_bytes("0xb8c472cf9e8b1b7b497df3a52cd5cea70b8a402c").unwrap() {
            33
        } else {
            29
        };
        assert!(error.contains(&format!("{scalar:064x}")));
...
#[test]
fn packed_fields_and_allowances_preserve_the_independent_rpc_balance() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/packed400/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn packed_raw_word_controls_match_rpc_including_null_address_filtering() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/packed400/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn packed_unreviewed_mapping_writes_still_stop_processing() {
    let layouts = layouts();
    for case in cases() {
        let contract = hex_bytes(case["contract"].as_str().unwrap()).unwrap();
        let layout = layouts.iter().find(|l| l.contract == contract).unwrap();
...
#[test]
fn packed_unreviewed_scalar_writes_still_stop_processing() {
    let layouts = layouts();
    for case in cases() {
        let contract = hex_bytes(case["contract"].as_str().unwrap()).unwrap();
        let layout = layouts.iter().find(|l| l.contract == contract).unwrap();
