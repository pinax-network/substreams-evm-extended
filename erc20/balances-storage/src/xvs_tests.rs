...
#[test]
fn xvs_unsigned_mapping_match_captured_independent_rpc() {
    let layouts = layout::parse(include_str!("../tests/fixtures/xvs/layouts.json")).unwrap();
    assert_eq!(layouts.len(), 1);
    let cases: Vec<Value> = serde_json::from_str(include_str!("../tests/fixtures/xvs/cases.json")).unwrap();
