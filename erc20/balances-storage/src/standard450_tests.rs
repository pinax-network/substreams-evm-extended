...
fn layouts() -> Vec<VerifiedLayout> {
    layout::parse(include_str!("../tests/fixtures/standard450/layouts.json")).unwrap()
}

fn cases() -> Vec<Value> {
    serde_json::from_str(include_str!("../tests/fixtures/standard450/cases.json")).unwrap()
}

fn captured(case: &Value) -> eth::Block {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/standard450");
    eth::Block::decode(std::fs::read(root.join(case["fixture"].as_str().unwrap())).unwrap().as_slice()).unwrap()
}

#[test]
fn standard450_captured_blocks_match_independent_rpc() {
    let layouts = layouts();
...
#[test]
fn standard450_fixed_fields_allowances_and_packed_controls_preserve_the_independent_rpc_balance() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/standard450/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn standard450_raw_word_controls_match_rpc_including_null_address_filtering() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/standard450/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
    ...
    assert_eq!(excluded_null, 20);
}

#[test]
fn standard450_unreviewed_mapping_writes_still_stop_processing() {
    let layouts = layouts();
    for case in cases() {
        let contract = hex_bytes(case["contract"].as_str().unwrap()).unwrap();
        let layout = layouts.iter().find(|l| l.contract == contract).unwrap();
...
#[test]
fn standard450_unknown_fixed_fields_remain_guarded() {
...
#[test]
fn twt_owner_and_decimals_share_the_reviewed_packed_word() {
    let controls: Value = serde_json::from_str(include_str!("../tests/fixtures/standard450/controls.json")).unwrap();
    let token = controls["tokens"].as_array().unwrap().iter().find(|t| t["rank"] == 450).unwrap();
    let mut checked = 0;
    for control in token["metadata_controls"].as_array().unwrap().iter().filter(|c| c["kind"] == "packed") {
        let value = hex_bytes(control["state_diff"][format!("0x{:064x}", 5)].as_str().unwrap()).unwrap();
        let fields = control["field_calls"].as_array().unwrap();
        assert_eq!(fields[0]["call"]["data"], "0x8da5cb5b");
        assert_eq!(fields[1]["call"]["data"], "0x313ce567");
        assert_eq!(BigInt::from_unsigned_bytes_be(&value[11..31]).to_string(), fields[0]["value"].as_str().unwrap());
        assert_eq!(value[31].to_string(), fields[1]["value"].as_str().unwrap());
