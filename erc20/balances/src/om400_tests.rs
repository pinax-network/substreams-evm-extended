...
fn layouts() -> Vec<VerifiedLayout> {
    layout::parse(include_str!("../tests/fixtures/om400/layouts.json")).unwrap()
}

fn cases() -> Vec<Value> {
    serde_json::from_str(include_str!("../tests/fixtures/om400/cases.json")).unwrap()
}

fn captured(case: &Value) -> eth::Block {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/om400");
    eth::Block::decode(std::fs::read(root.join(case["fixture"].as_str().unwrap())).unwrap().as_slice()).unwrap()
}

#[test]
fn om_captured_minimal_proxy_block_matches_independent_rpc() {
    let layouts = layouts();
...
#[test]
fn om_fixed_fields_and_allowances_preserve_the_independent_rpc_balance() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/om400/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn om_raw_word_controls_match_rpc_including_null_address_filtering() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/om400/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn om_unreviewed_mapping_writes_still_stop_processing() {
    let layouts = layouts();
    for case in cases() {
        let contract = hex_bytes(case["contract"].as_str().unwrap()).unwrap();
        let layout = layouts.iter().find(|l| l.contract == contract).unwrap();
...
#[test]
fn om_implementation_code_changes_are_rejected_without_holder_writes() {
    let layout = layouts().remove(0);
    let mut block = captured(&cases()[0]);
    block.transaction_traces.clear();
    block.system_calls.clear();
    let implementation = layout.minimal_proxy.as_ref().unwrap();
    block.code_changes = vec![eth::CodeChange {
        address: implementation.implementation.clone(),
        old_hash: implementation.code_hash.to_vec(),
