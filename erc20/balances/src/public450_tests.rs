...
fn layouts() -> Vec<VerifiedLayout> {
    layout::parse(include_str!("../tests/fixtures/public450/layouts.json")).unwrap()
}

fn cases() -> Vec<Value> {
    serde_json::from_str(include_str!("../tests/fixtures/public450/cases.json")).unwrap()
}

fn captured(case: &Value) -> eth::Block {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/public450");
    eth::Block::decode(std::fs::read(root.join(case["fixture"].as_str().unwrap())).unwrap().as_slice()).unwrap()
}

#[test]
fn public450_captured_blocks_match_independent_rpc() {
    let layouts = layouts();
...
#[test]
fn public450_fixed_fields_allowances_and_records_preserve_the_independent_rpc_balance() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/public450/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn public450_raw_word_controls_match_rpc_including_null_address_filtering() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/public450/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
...
#[test]
fn public450_unreviewed_mapping_writes_still_stop_processing() {
    let layouts = layouts();
    for case in cases() {
        let contract = hex_bytes(case["contract"].as_str().unwrap()).unwrap();
        let layout = layouts.iter().find(|l| l.contract == contract).unwrap();
...
#[test]
fn public450_packed_metadata_is_required_and_unknown_fixed_fields_reject() {
    let configured = layouts();
    let controls: Value = serde_json::from_str(include_str!("../tests/fixtures/public450/controls.json")).unwrap();
    let mut checked = 0;
