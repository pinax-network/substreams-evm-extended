...
#[test]
fn ranks251_to300_qualified_profiles_match_captured_independent_rpc() {
    let layouts = layout::parse(include_str!("../tests/fixtures/ranks251-300/layouts.json")).unwrap();
    assert_eq!(layouts.len(), 33);
    let cases: Vec<Value> = serde_json::from_str(include_str!("../tests/fixtures/ranks251-300/cases.json")).unwrap();
    ...
    for case in cases {
        ...
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/ranks251-300")
