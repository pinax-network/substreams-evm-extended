...
#[test]
fn remaining_top350_reviewed_profiles_match_captured_independent_rpc() {
    let layouts = layout::parse(include_str!("../tests/fixtures/pending350/layouts.json")).unwrap();
    assert_eq!(layouts.len(), 13);
    let cases: Vec<Value> = serde_json::from_str(include_str!("../tests/fixtures/pending350/cases.json")).unwrap();
    ...
    for case in cases {
        ...
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/pending350")
