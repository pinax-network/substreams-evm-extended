...
fn layouts() -> Vec<VerifiedLayout> {
    layout::parse(include_str!("../tests/fixtures/seven400/layouts.json")).unwrap()
}

fn cases() -> Vec<Value> {
    serde_json::from_str(include_str!("../tests/fixtures/seven400/cases.json")).unwrap()
}

fn captured(case: &Value) -> eth::Block {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/seven400");
    eth::Block::decode(std::fs::read(root.join(case["fixture"].as_str().unwrap())).unwrap().as_slice()).unwrap()
}

#[test]
fn seven_captured_blocks_match_independent_rpc() {
    let layouts = layouts();
...
#[test]
fn seven_fixed_fields_allowances_and_nonces_preserve_the_independent_rpc_balance() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/seven400/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
    for token in fixture["tokens"].as_array().unwrap() {
        ...
        for control in token["metadata_controls"].as_array().unwrap() {
            let mut preimages = std::collections::HashMap::from([(hex::encode(hash(&preimage)), hex::encode(preimage))]);
            if matches!(control["kind"].as_str(), Some("allowance" | "nonce")) {
                let call = hex_bytes(control["field_call"]["data"].as_str().unwrap()).unwrap();
                assert_eq!(&call[16..36], holder);
                let mut owner = [0; 64];
                owner[..32].copy_from_slice(&call[4..36]);
                owner[63] = control["slot"].as_u64().unwrap().try_into().unwrap();
                let owner_key = hash(&owner);
                preimages.insert(hex::encode(owner_key), hex::encode(owner));
                let key = if control["kind"] == "allowance" {
                    let mut spender = [0; 64];
                    spender[..32].copy_from_slice(&call[36..68]);
                    spender[32..].copy_from_slice(&owner_key);
                    let key = hash(&spender);
                    preimages.insert(hex::encode(key), hex::encode(spender));
                    key
                } else {
                    owner_key
                };
                assert!(control["state_diff"].get(format!("0x{}", hex::encode(key))).is_some());
            }
...
#[test]
fn seven_raw_word_controls_match_rpc_including_null_address_filtering() {
    let fixture: Value = serde_json::from_str(include_str!("../tests/fixtures/seven400/controls.json")).unwrap();
    let layouts = layouts();
    let mut checked = 0;
    ...
    assert_eq!(excluded_null, 28);
}

#[test]
fn seven_unreviewed_mapping_writes_still_stop_processing() {
    let layouts = layouts();
    for case in cases() {
        let contract = hex_bytes(case["contract"].as_str().unwrap()).unwrap();
        let layout = layouts.iter().find(|l| l.contract == contract).unwrap();
...
#[test]
fn mos_packed_swap_field_must_be_independently_reviewed() {
    let contract = hex_bytes("0xc9c8050639c4cc0df159e0e47020d6e392191407").unwrap();
    let case = cases()
        .into_iter()
        .find(|c| hex_bytes(c["contract"].as_str().unwrap()).unwrap() == contract)
        .unwrap();
    let block = captured(&case);
    let mut layout = layouts().into_iter().find(|l| l.contract == contract).unwrap();
    let key = word(&[11]).unwrap();
    assert!(layout.other_slots.remove(&key));
    let error = project(&block, &[layout]).unwrap_err().to_string();
    assert!(error.contains("unresolved storage") && error.contains(&hex::encode(key)));
}
