...
cargo run --locked -p erc20-balances-tools -- rank-tokens \
  ...
  --output erc20/balances/out/recheck-ranking

cargo run --locked -p erc20-balances-tools -- capture-blocks \
  --ranking erc20/balances/out/recheck-ranking/report.json \
  --samples-per-token 8 --output erc20/balances/out/recheck-samples

cargo run --locked -p erc20-balances-tools -- capture-blocks \
  --start 122288006 --blocks 64 \
  --output erc20/balances/out/recheck-continuous

cargo run --locked -p erc20-balances-tools -- test-ranked \
  --ranking erc20/balances/out/recheck-ranking/report.json \
  --block-dir erc20/balances/out/recheck-samples \
  --block-dir erc20/balances/out/recheck-continuous \
  --layouts erc20/balances/tests/fixtures/bsc-reviewed-layouts.json \
  --output erc20/balances/out/recheck-survey

cargo run --locked -p erc20-balances-tools -- audit-rpc \
  --start 122288006 --blocks 64 \
  --layouts erc20/balances/tests/fixtures/bsc-reviewed-layouts.json \
  --output erc20/balances/out/recheck-wasm

cargo run --locked -p erc20-balances-tools -- holder-coverage \
  --ranking erc20/balances/out/recheck-ranking/report.json \
  --block-dir erc20/balances/out/recheck-continuous \
  --layouts erc20/balances/tests/fixtures/bsc-reviewed-layouts.json \
  --output erc20/balances/out/recheck-holders
```
